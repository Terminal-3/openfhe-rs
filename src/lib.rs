...
pub mod ffi {
    ...
    unsafe extern "C++" {
        fn GenILDCRTParamsByOrderSizeBits(
            ...
            depth: u32,
            bits: u32,
        ) -> UniquePtr<ILDCRTParams>;
    }

    unsafe extern "C++" {
        // VectorOfCiphertexts
        fn Len(self: &VectorOfCiphertexts) -> usize;
        /// build an empty VectorOfCiphertexts
        fn vector_of_ciphertexts_single(
            ct: &SharedPtr<CiphertextImpl>,
        ) -> UniquePtr<VectorOfCiphertexts>;

        fn GetRef(self: &CiphertextDCRTPoly) -> &SharedPtr<CiphertextImpl>;

        fn vector_of_ciphertexts_empty() -> UniquePtr<VectorOfCiphertexts>;

        fn vector_of_ciphertexts_extend(
            dest: Pin<&mut VectorOfCiphertexts>,
            src: &VectorOfCiphertexts,
        );
    }
